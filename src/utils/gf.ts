...
export const addDateRange = (params: any, dateRange: any[], propName?: string) => {
  ...
  if (typeof propName === 'undefined') {
    search.params['createTime'] = dateRange[0] + "," + dateRange[1];
  } else {
    search.params[propName] = dateRange[0] + "," + dateRange[1];
  }
  return search;
};
 ...
 * @param propName
 */
export function btnShow(status: string, type?: string) {
  if (type == 'del' && (status == 'draft' || status == 'cancel')) { // 删除按钮：draft、cancel、back、
    return true
  } else if (type == 'view' && (status == 'waiting' || status == 'termination' || status == 'finish' || status == 'open')) { //查看按钮：
    return true
  } else if (type == 'edit' && (status == 'draft' || status == 'cancel' || status == 'back')) { //编辑按钮：draft、cancel、back、
    return true
  } else if (type == 'undo' && (status == 'waiting')) { //撤销按钮： 待审核
    return true
  } else if (type == 'progress' && status != 'draft' && status != 'cancel') { //进度按钮：除”草稿、撤销“都可
    return true
  } else if (type == 'stop' && (status == 'waiting' || status == 'back' || status == 'open')) { //终止任务： 除”已完成、草稿 “都可
    return true
  } else {
    return false
  }
